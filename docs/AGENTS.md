...
| ⭐ drive herdr in the base from an agent on Windows | `wsl-toolkit base herdr -- ...`. ⚠ The herdr the base pins, 0.9.0, has no `--machine`: it exits 2 on the flag | `base exec -c 'herdr ...'`, which composes a shell string out of a prompt that is prose |
