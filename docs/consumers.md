...
⭐ **TWO FILES HERE ARE MEANT TO BE FETCHED AND HAVE NO CONSUMER ROW YET.**
[`../scripts/common/bootstrap.sh`](../scripts/common/bootstrap.sh) and
[`../scripts/common/tmux.conf`](../scripts/common/tmux.conf) exist to be pulled
into a guest by URL, the way the operator already pulls
`Arsenal/misc/Linux/Debian/bootstrap.sh` and a `.bashrc` from
`pkgforge/devscripts`. Nothing is known to fetch them today, so there is no row
above rather than a speculative one. ⚠ They are governed by every rule on this
page from the moment anything does: the bootstrap carries a `--version` that
prints a contract version for exactly that reason, and its report is `key=value`
lines plus an optional `--json` object so a caller has something stable to read.
