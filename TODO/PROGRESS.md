...
101. ⭐ **THE MUTATION JOB WAS 26 MINUTES ON EVERY PUSH, AND IT IS THE COPY.**
     ...
     **1.49 s**. That module holds **332 of 419 rows**, so the copy alone is
     ...
     ⚠ **CI MEASURED THREE TIMES AND ONLY THE FIRST WAS DRAMATIC**: 19.7, 24.0
     and 24.4 minutes, against 26.4, 26.6 and 27.2 before. That is a mean of
     about 26.7 against 22.7, on a shared runner, and two of the three sit close
     to the old range. ⛔ **So the record does NOT claim a CI percentage**: the
     firm figure is the per-row one measured on the same 20 rows, and the CI
     numbers are given in full so a reader can judge them.
