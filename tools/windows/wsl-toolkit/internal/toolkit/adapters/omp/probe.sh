...
# launcher FIRST and the wrapper on the system path is never reached. omp keeps its startup model and effort in its own configuration, so those survive; the account guard does not.
# ⚠ A probe that only asked whether the name resolved called that base healthy.
if [ -n "$pane_path" ]; then
  if head -3 "$pane_path" 2>/dev/null | grep -q "wsl-toolkit's omp adapter"; then
    printf 'wrapper_on_login_path yes
'
  else
    printf 'wrapper_on_login_path no
'
    problem "a login shell resolves omp to $pane_path, which is not this tool's wrapper. The account's own prefix is ahead of /usr/local/bin, which bootstrap.sh puts there, so the wrapper and everything it sets are bypassed"
  fi
fi
