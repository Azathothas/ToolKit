# wsl-ephemeral-selftest.ps1

Run [`wsl-ephemeral.ps1`](wsl-ephemeral.md)'s pure functions against a table of
...
```text
wsl-ephemeral-selftest: 63 case(s) passed over 15 function(s) loaded from wsl-ephemeral.ps1.
```
...
⭐ **Most of `wsl-ephemeral.ps1` can only be proved by driving a real distro**,
...
⛔ **`wsl-ephemeral.ps1` cannot be dot-sourced.** Its top level dispatches an
...
| | |
| --- | --- |
| exit codes | 0 pass, 1 fail, ⚠ 2 only when `wsl-ephemeral.ps1` is not beside it |
| `-Json` | `{"schema":"wsl-ephemeral-selftest/1","cases":63,"failed":0,"functions":15}` |
...
| | |
| --- | --- |
| the local gate | ⭐ both halves, as `wsl-ephemeral selftest` |
...
- [`wsl-ephemeral.md`](wsl-ephemeral.md), the tool under test.
